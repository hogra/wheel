...
let W = new Float32Array(res * res);
let newW = new Float32Array(res * res);
let S = new Float32Array(res * res);
let U = new Float32Array(res * res);
let V = new Float32Array(res * res);
let mask = new Float32Array(res * res);
...
// Генерация вихрей строго на твердой стенке
...
// Расчет скоростей из Функции Тока (u = dS/dy, v = -dS/dx)
...
// Перенос вихрей воздухом (Advection) + Диффузия (Вязкость)
    ...
    for (let i = 1; i < res - 1; i++) {
        for (let j = 1; j < res - 1; j++) {
            if (mask[ix(i, j)] === 1.0) {
                ...
                // безопасный коридор от аномалий чисел с плавающей точкой.
